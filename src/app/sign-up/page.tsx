...
export default function SignUpPage() {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .left-side {
            ...
            background: linear-gradient(to right, rgba(10, 10, 10, 0.1) 50%, var(--bg-dark) 100%),
                url(/asset/background-sign-up.png) center/cover no-repeat;
        }
        .header-overlay {
            position: absolute;
            top: 0;
                  ...
                  <div className="form-card">
                      ...
                      <form action={formAction}>
                          <div className="form-group">
                              ...
                              {state?.errors?.nama && <p style={{color: 'red', fontSize: '13px', marginTop: '5px'}}>{state.errors.nama[0]}</p>}
                          </div>
                          <div className="form-row">
                              <div className="form-group">
                                  <label>Nomor Whatsapp</label>
                                  <input type="text" name="whatsapp" />
                                  {state?.errors?.whatsapp && <p style={{color: 'red', fontSize: '13px', marginTop: '5px'}}>{state.errors.whatsapp[0]}</p>}
                              </div>
                              <div className="form-group">
                                  <label>E-mail</label>
                                  <input type="email" name="email" />
                                  {state?.errors?.email && <p style={{color: 'red', fontSize: '13px', marginTop: '5px'}}>{state.errors.email[0]}</p>}
                              </div>
                          </div>
                          <div className="form-group">
                          ...
                          <button type="submit" className="btn-submit" disabled={isPending}>
                              {isPending ? (
                                <span><i className="fa-solid fa-spinner fa-spin" style={{marginRight: '8px'}}></i> Memproses...</span>
                              ) : (
                                'Daftar Sekarang'
                              )}
                          </button>
      ...
      {showModal && (
        <div style={{ display: 'flex', position: 'fixed', zIndex: 9999, left: 0, top: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ backgroundColor: '#1f1f1f', padding: '40px 30px', borderRadius: '16px', width: '90%', maxWidth: '400px', textAlign: 'center', border: '1px solid #333' }}>
                ...
                <p style={{ color: '#aaa', marginBottom: '32px', fontSize: '15px', lineHeight: 1.6 }}>Akun Anda berhasil dibuat di Supabase. Silakan login untuk melanjutkan.</p>
                <button onClick={() => window.location.href = '/login'} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', backgroundColor: '#bdd124', color: '#000', fontSize: '16px', fontWeight: 600, cursor: 'pointer', transition: '0.3s' }}>Lanjut ke Login</button>
            </div>
        </div>
      )}
    </>
